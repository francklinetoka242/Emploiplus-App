...
serve(async (req) => {
  ...
  try {
    ...
    if (!emailResponse.ok) {
      ...
      // Don't throw error - code was generated successfully, email might fail
      // but we'll return success so user can retry
    }
    ...
    return new Response(
      JSON.stringify({
        ...
        code, // Remove in production, only for testing
      }),
